...
function F = root2d(x) %starts fSolve equation
...
zprime2 = atand(a2 + b2)
m2 = sqrt(x2^2+z2^2); %magnitude based on these distances
...
%moment equation
F(3) = ((x2 - x(1)) * zprime2)/m2 - (x2*z2)/m2 - x(1);

%Trials 2 and 3 repeat the same process of trial 1.
...
%Fx=0 equation
A(1) = (y(1) * x2)/m2 - (y(2) * z4)/m4;

%Fz=0 equation
A(2) = (y(1)*zprime2)/m2 + (y(2)*zprime4)/m4 - HW;

%moment equation
A(3) = ((x2 - y(1)) * zprime2)/m2 - (x2*z2)/m2 - y(1);
...
Radii = [x(1), y(1), z(1)]
disp('Estimated tension value for pulley 2 from Trial 1, 2, and 3')
Tension_2 = [x(2), y(2), z(2)]
disp('Estimated tension value for pulley 4 from Trial 1, 2, and 3')
Tension_4 = [x(3), y(3), z(3)]

Average = mean(Radii) %Standard Deviation calculation
StanDev = ((sum((Radii-Average).^2))./(3))^.5;
StanDevRange = [Average - 3*StanDev, Average + 3*StanDev];

formatSpec = ['The Standard Deviation is %f2 meters. ' ... %displays standard deviation
